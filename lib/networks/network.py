...
class Network(object):
    ...
    def softmax(self, input, name):
        input_shape = tf.shape(input)
        if name == 'rpn_cls_prob' or name=='attention_map2':
    ...
    # @layer
    # def attention_refine_layer(self, input, name):
    #     # inputs is feature maps which are going to be refined using attention map.
    #     if isinstance(input[0], tuple):
    #         input[0] = input[0][0]
    #     with tf.variable_scope(name) as scope:
    #         attention_feat = tf.py_func(attention_refine_layer_py, [input[0],input[1]], [tf.float32])
    #         attention_feat = tf.convert_to_tensor(attention_feat, name=name)
    #         return attention_feat

    @layer
    def attention_refine_layer(self,input,name):
        feat_shape = input[0].get_shape()
        print(input[0])
        print(feat_shape)
        feat = input[0]
        att_map = input[1]
        print(feat)
        att_map_shape = input[1].get_shape()
        att_map = tf.tile(att_map,[1,1,1,int(feat_shape[3])])
        return tf.multiply(feat,att_map,name=name)

    @layer
    def attention_refine_layer2(self,input,name):
        feat_shape = input[0].get_shape()
        print(input[0])
        print(input[1])
        print(feat_shape)
